...
export type BaseStrategyEventArgs = BaseStrategyHarvestedEventArgs;

export enum BaseStrategyEvents {
  Harvested = "Harvested",
}

export interface BaseStrategyHarvestedEventArgs extends DecodedLogArgs {
  profit: BigNumber;
}
...
export class BaseStrategyContract extends BaseContract {
  /**
   * @ignore
   */
  public static deployedBytecode: string | undefined;
  public static contractName = "BaseStrategy";
  private readonly _methodABIIndex: { [name: string]: number } = {};
  private readonly _subscriptionManager: SubscriptionManager<
    BaseStrategyEventArgs,
    BaseStrategyEvents
  >;
  public static async deployFrom0xArtifactAsync(
    ...
    return BaseStrategyContract.deployAsync(
      bytecode,
      abi,
      provider,
      txDefaults,
  ...
  public static async deployWithLibrariesFrom0xArtifactAsync(
    ...
    const libraryAddresses = await BaseStrategyContract._deployLibrariesAsync(
      artifact,
      libraryArtifacts,
  ...
  public static async deployAsync(
    ...
    logUtils.log(
      `BaseStrategy successfully deployed at ${txReceipt.contractAddress}`
    );
    const contractInstance = new BaseStrategyContract(
    ...
    for (const link of Object.values(links)) {
      for (const libraryName of Object.keys(link)) {
        if (!libraryAddresses[libraryName]) {
          ...
          await BaseStrategyContract._deployLibrariesAsync(
            libraryArtifact,
            libraryArtifacts,
            web3Wrapper,
            txDefaults,
  ...
  public getFunctionSignature(methodName: string): string {
    ...
    const methodAbi = BaseStrategyContract.ABI()[index] as MethodAbi; // tslint:disable-line:no-unnecessary-type-assertion
  ...
  public getABIDecodedTransactionData<T>(
    ...
    const self = (this as any) as BaseStrategyContract;
  ...
  /**
   * Subscribe to an event type emitted by the BaseStrategy contract.
   * @param eventName The BaseStrategy contract event you would like to subscribe to.
   ...
   */
  public subscribe<ArgsType extends BaseStrategyEventArgs>(
    eventName: BaseStrategyEvents,
    indexFilterValues: IndexedFilterValues,
    callback: EventCallback<ArgsType>,
    isVerbose: boolean = false,
    blockPollingIntervalMs?: number
  ): string {
    assert.doesBelongToStringEnum("eventName", eventName, BaseStrategyEvents);
   ...
   */
  public async getLogsAsync<ArgsType extends BaseStrategyEventArgs>(
    eventName: BaseStrategyEvents,
  ...
  constructor(
    ...
    txDefaults?: Partial<TxData>,
    logDecodeDependencies?: { [contractName: string]: ContractAbi },
    deployedBytecode: string | undefined = BaseStrategyContract.deployedBytecode
  ) {
    super(
      "BaseStrategy",
      BaseStrategyContract.ABI(),
    ...
    this._subscriptionManager = new SubscriptionManager<
      BaseStrategyEventArgs,
      BaseStrategyEvents
    >(BaseStrategyContract.ABI(), this._web3Wrapper);
    BaseStrategyContract.ABI().forEach((item, index) => {
